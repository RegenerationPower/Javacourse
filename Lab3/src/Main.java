...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        if(text.indexOf(symbol)!=-1) {
            Set<Integer> symbols = symbol.chars().boxed().collect(Collectors.toSet());
            String text2 = text.toString();
            Set<String> words = Arrays.stream(text2.split("[\\p{Punct}\\s]+")).map(String::toLowerCase).collect(Collectors.toSet());
            Map<String, Long> symbolCount = words.stream().collect(Collectors.toMap(Function.identity(), w -> w.chars().filter(symbols::contains).count()));
            List<String> orderedWords = words.stream().sorted((a, b) -> (int) (symbolCount.get(b) - symbolCount.get(a))).collect(Collectors.toList());
